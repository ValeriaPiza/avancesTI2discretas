...
object Main {
  ...
  private def runWithRealData(datasetPath: String): Unit = {
    ...
    if (results.nonEmpty) {
      ...
      println("📊 Datos para gráficas: results/experiment_results.csv")
  ...
  private def printGeneratedFiles(): Unit = {
    ...
    if (graficasDir.exists() && graficasDir.isDirectory) {
      ...
      if (files != null && files.nonEmpty) {
        ...
        println("\n⚠️  No se generaron archivos en results/graficas/")
    ...
    try {
      ...
      if (pythonScript.exists()) {
        ...
        if (exitCode == 0) {
          ...
          println("⚠️  Las gráficas no se pudieron generar automáticamente")
